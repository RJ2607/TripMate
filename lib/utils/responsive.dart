...
class Responsive {
  ...
  Responsive._({required this.context})
      : deviceHeight = MediaQuery.maybeOf(context)?.size.height ?? 0,
        deviceWidth = MediaQuery.maybeOf(context)?.size.width ?? 0;
...
// general size
  Size get screenSize => MediaQuery.maybeOf(context)?.size ?? Size(0, 0);
  EdgeInsets get screenPadding => MediaQuery.maybeOf(context)?.padding ?? EdgeInsets.zero;
  ...
  double setTextScaleFactor({required double textScaleFactor}) {
    final screenTextScaleFactor = MediaQuery.maybeOf(context)?.textScaleFactor ?? 1.0;
